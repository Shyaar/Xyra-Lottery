...
contract LotteryManager is ReentrancyGuard, Ownable {
    ...
    constructor(IWETH _weth, TokenVault _vault) Ownable(msg.sender) {
        if (address(_weth) == address(0)) revert LotteryErrors.ZeroAddress();
        if (address(_vault) == address(0)) revert LotteryErrors.ZeroAddress();

        WETH = _weth;
        VAULT = _vault;
        ASSET = IERC20(address(_weth));
        VAULT_SHARE = IERC20(address(VAULT));
    ...
    // -------------------------------
    // Ticket & Lottery Functions
    // -------------------------------
    function buyTicket() external payable nonReentrant onlyActive {
        if (msg.value == 0) revert LotteryErrors.MustSendETH();
        ...
        emit LotteryEvents.TicketBought(msg.sender, msg.value, sharesReceived);
    }
    ...
    function receiveRandomness(
        uint256 requestId,
        uint256 randomness
    ) external onlyRandomifier nonReentrant {
    ...
    function claimPrincipal() external nonReentrant onlyEnded {
        ...
        if (amountReceived > 0) {
            ...
            if (!sent) revert LotteryErrors.EthTransferFailed();
        }
    ...
    function claimPrize() external nonReentrant onlyEnded {
        ...
        WETH.withdraw(prizeAmountRedeemed);
        (bool sent, ) = payable(winner).call{value: prizeAmountRedeemed}("");
        if (!sent) revert LotteryErrors.EthTransferFailed();
