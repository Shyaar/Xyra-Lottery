...
const contractAddress = process.env.NEXT_PUBLIC_LOTTERY_MANAGER_CONTRACT_ADDRESS as `0x${string}`;
...
export function useBuyTicketWrite() {
  ...
  useEffect(() => {
    ...
    if (isWriteError && !toastShownRef.current.error) {
      ...
      toast.error(`❌ Error buying ticket: ${writeError?.message || "Unknown error"}`);
  ...
  useEffect(() => {
    ...
    if (isConfirmError && !toastShownRef.current.error) {
      ...
      toast.error(`❌ Error confirming ticket purchase: ${confirmError?.message || "Unknown error"}`);
      toastShownRef.current.error = true;
    }
